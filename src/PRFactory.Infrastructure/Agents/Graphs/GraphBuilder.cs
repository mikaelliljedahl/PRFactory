...
public class CheckpointNode : GraphNode
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var checkpointStore = ServiceProvider.GetRequiredService<Base.ICheckpointStore>();

        context.State["checkpoint_name"] = CheckpointName;
        context.State["checkpoint_time"] = DateTime.UtcNow;

        await checkpointStore.SaveCheckpointAsync(context.TicketId, context.GraphId, CheckpointName, context.State);
