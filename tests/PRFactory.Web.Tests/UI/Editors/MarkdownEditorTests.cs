...
{
    [Fact]
    public void Render_DisplaysAllFormattingButtons()
    {
        // Arrange & Act
        ...
        Assert.Contains("Strikethrough", cut.Markup);
    }

    [Fact]
    public void HeadingDropdown_DisplaysAllHeadingOptions()
    {
        // Arrange & Act
        var cut = RenderComponent<MarkdownToolbar>();

        // Assert
        var headingSelect = cut.Find("select");
        Assert.Contains("H1", headingSelect.InnerHtml);
        Assert.Contains("H2", headingSelect.InnerHtml);
        Assert.Contains("H3", headingSelect.InnerHtml);
        Assert.Contains("H4", headingSelect.InnerHtml);
        Assert.Contains("H5", headingSelect.InnerHtml);
        Assert.Contains("H6", headingSelect.InnerHtml);
    }

    [Fact]
    public void ViewModeDropdown_DisplaysAllViewModes()
    {
        // Arrange & Act
        var cut = RenderComponent<MarkdownToolbar>();

        // Assert
        var viewModeSelects = cut.FindAll("select");
        var viewModeSelect = viewModeSelects[viewModeSelects.Count - 1];
        Assert.Contains("Split View", viewModeSelect.InnerHtml);
        Assert.Contains("Editor Only", viewModeSelect.InnerHtml);
        Assert.Contains("Preview Only", viewModeSelect.InnerHtml);
        Assert.Contains("Fullscreen", viewModeSelect.InnerHtml);
    }

    [Fact]
    public void BoldButton_HasCorrectTitle()
    {
        // Arrange & Act
        var cut = RenderComponent<MarkdownToolbar>();
    ...
    [Fact]
    public void ListButtons_ArePresent()
    {
        // Arrange & Act
        var cut = RenderComponent<MarkdownToolbar>();

        // Assert
        Assert.NotNull(cut.Find("button[title*='Bullet List']"));
        Assert.NotNull(cut.Find("button[title*='Numbered List']"));
        Assert.NotNull(cut.Find("button[title*='Checklist']"));
    }

    [Fact]
    public void LinkButton_HasCorrectTitle()
    {
        // Arrange & Act
        var cut = RenderComponent<MarkdownToolbar>();

        // Assert
        var linkButton = cut.Find("button[title*='Insert Link']");
        Assert.NotNull(linkButton);
        Assert.Contains("Ctrl+K", linkButton.GetAttribute("title"));
    }

    [Fact]
    public void InsertButtons_ArePresent()
    {
        // Arrange & Act
        var cut = RenderComponent<MarkdownToolbar>();

        // Assert
        Assert.NotNull(cut.Find("button[title*='Insert Image']"));
        Assert.NotNull(cut.Find("button[title*='Code Block']"));
        Assert.NotNull(cut.Find("button[title*='Insert Table']"));
    }
}
